...
def load_death_rate_data() -> pd.DataFrame:
    """
    Загружает данные о смертности из Excel файла.
    
    Returns:
        DataFrame с данными о смертности
    """
...
def process_death_rate_data(df: pd.DataFrame, countries: List[str]) -> pd.DataFrame:
    """
    Обрабатывает данные о смертности для визуализации.
...
def create_death_rate_plot(
    df: pd.DataFrame,
    country_labels: Dict[str, str],
    save_path: str = "./images/death_rate.png"
) -> None:
    """
    Создает график смертности по странам.
    ...
    plt.title("Динамика смертности по странам", pad=20)
...
def main():
    ...
    try:
        ...
        df = load_death_rate_data()
        if df.empty:
            raise ValueError("Не удалось загрузить данные о смертности")
            
        df = process_death_rate_data(df, countries)
        
        # Создаем визуализацию
        create_death_rate_plot(df, country_labels)
        
        print("График сохранен в death_rate.png")
